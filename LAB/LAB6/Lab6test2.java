...
public class Lab6test2 {
    ...
    static class Char {
        ...
        int compare(Char other) {
            if (!this.tier.equals(other.tier)) {
                return -this.tier.compareTo(other.tier); // Descending order for tier
            }
    ...
    static class AVLTree {
        ...
        boolean delete(Char character) {
            int originalSize = countNodes(root);
        ...
        void getTopThree(Node node, List<Char> list) {
            ...
            getTopThree(node.left, list); // Left first for higher tiers
            if (list.size() < 3) list.add(node.character);
            getTopThree(node.right, list);
        }

        void getBottomThree(Node node, List<Char> list) {
            if (node == null || list.size() >= 3) return;
            getBottomThree(node.right, list); // Right first for lower tiers
            if (list.size() < 3) list.add(node.character);
            getBottomThree(node.left, list);
        }
