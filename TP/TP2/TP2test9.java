    ...
    private static Participant getOrCreateParticipant(int participantId, Team team) {
        ...
        if (obj instanceof ParticipantRange) {
            ...
            if (participantId >= range.startId && participantId <= range.endId) {
                ...
                long count = team.pointCounts.get(range.points) - 1;
                if (count == 0) {
                    team.pointCounts.remove(range.points);
                } else {
                    team.pointCounts.put(range.points, count);
                }

                // Increase unique point count if necessary
                if (!team.pointCounts.containsKey(range.points)) {
                    team.uniquePointCount = team.pointCounts.size();
                }

                // Add participant's points to point counts
                team.pointCounts.put(participant.points, team.pointCounts.getOrDefault(participant.points, 0L) + 1);
                team.uniquePointCount = team.pointCounts.size();
