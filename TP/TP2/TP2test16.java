...
public class TP2test16 {
        ...
        for (int q = 0; q < Q; q++) {
            ...
            if (command == 'A'){
                ...
                if (teamList.size() != 0) {
                    ...
                    // Move Sofita in the specified direction
                    ...
                    do {
                        ...
                        if (jokiTeam != null && sofitaTeam.id == jokiTeam.id) {
                            ...
                            // Joki moves to the nearest team with lowest points
                            int jokiIndex = teamList.indexOf(jokiTeam);
                            jokiTeam = getNearestTeamWithLowestPoints(teamList, sofitaTeam, jokiIndex, direction);
                        }
                    ...
                    } while (continueMoving && sofitaTeam != null);
                ...
                if (sender == null || receiver == null || sender.team != sofitaTeam || receiver.team != sofitaTeam) {
                    ...
                    if (sender.team.memberCount < 7) {
                        ...
                        // If Sofita's team is eliminated, she moves to the team with the highest total points
                        if (sender.team == sofitaTeam) {
                            if (!teamList.isEmpty()) {
                ...
                if (!teamList.isEmpty()) {
                    // Update Sofita's team to the team with the highest total points
                    ...
                    if (jokiTeam != null && sofitaTeam.id == jokiTeam.id) {
                        ...
                        // Joki moves to the team with the lowest total points
                        int jokiIndex = teamList.indexOf(jokiTeam);
                        jokiTeam = getNearestTeamWithLowestPoints(teamList, sofitaTeam, jokiIndex, "R");
                    }
                ...
                if (jokiTeam != null && teamList.size() != 0) {
                    int jokiIndex = teamList.indexOf(jokiTeam);
                    // Move Joki in the specified direction
                    ...
                    if (destinationTeam != sofitaTeam) {
                        jokiTeam = destinationTeam;
                    }
                    out.println(jokiTeam != null ? jokiTeam.id : -1);
                } else {
                    out.println(-1);
                }
            }
    ...
    private static Team getNearestTeamWithLowestPoints(MyLinkedList<Team> teamList, Team excludeTeam, int jokiIndex, String direction) {
        int n = teamList.size();
        int minTotalPoints = Integer.MAX_VALUE;
    
        // First, find the minimum total points among teams (excluding excludeTeam)
        for (int i = 0; i < n; i++) {
            Team team = teamList.get(i);
            if (team == excludeTeam)
                continue;
            if (team.totalPoints < minTotalPoints) {
                minTotalPoints = team.totalPoints;
            }
        }
    
        // Collect indices of teams with the minimum total points (excluding excludeTeam)
        ...
        // Now find the team among candidates that is nearest to Joki's current position
        int minDistance = n; // Maximum possible distance in a circular list
        int bestIndex = -1;
        for (int idx : candidateIndices) {
            int dist = Math.min((idx - jokiIndex + n) % n, (jokiIndex - idx + n) % n);
    
            if (dist < minDistance) {
                minDistance = dist;
                bestIndex = idx;
            } else if (dist == minDistance) {
                // Tie-breaker: use direction preference
                if (direction.equals("L")) {
                    int leftIndex = (jokiIndex - dist + n) % n;
                    if (idx == leftIndex) {
                        bestIndex = idx;
                    }
                } else if (direction.equals("R")) {
                    int rightIndex = (jokiIndex + dist) % n;
                    if (idx == rightIndex) {
                        bestIndex = idx;
                    }
                }
        ...
        if (bestIndex != -1) {
            return teamList.get(bestIndex);
        } else {
            return null; // Should not happen unless all teams are excluded
        }
    }
