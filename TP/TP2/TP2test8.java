...
public class TP2test8 {
    ...
    // New class to represent a range of participants
    ...
    static class Team {
        ...
        long totalPoints; // Changed to long to prevent overflow
        long memberCount; // Changed to long to handle large counts
        int uniquePointCount;
        Map<Integer, Long> pointCounts; // Map to handle large counts
        int timesCaught;
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int q = 0; q < Q; q++) {
            ...
            switch (command) {
                case 'A':
                    // Efficiently add new participants as a range
                ...
                case 'V':
                    ...
                    Participant participant2 = getOrCreateParticipant(participant2Id, teams.get(opponentTeamId));

                    if (participant1 == null || participant2 == null || participant1.team != sofitaTeam || participant2.team != teams.get(opponentTeamId)) {
                        out.println(-1);
                    } else {
                ...
                case 'B':
                    ...
                    long[] pointsArray = new long[(int) K];
                    int idx = 0;

                    // Collect points from participants and participant ranges
                    for (int i = 0; i < sofitaTeam.participants.size(); i++) {
                        Object obj = sofitaTeam.participants.get(i);
                        if (obj instanceof Participant) {
                            Participant p = (Participant) obj;
                            pointsArray[idx++] = p.points;
                        } else if (obj instanceof ParticipantRange) {
                            ParticipantRange pr = (ParticipantRange) obj;
                            for (int j = 0; j < pr.endId - pr.startId + 1; j++) {
                                pointsArray[idx++] = pr.points;
                            }
                        }
                    }

                    // Sort the pointsArray
                    mergeSort(pointsArray, 0, pointsArray.length - 1);

                    int indexQ1 = (int) Math.floor(0.25 * K);
                    int indexQ3 = (int) Math.floor(0.75 * K);

                    long Q1 = pointsArray[Math.max(0, indexQ1 - 1)];
                    long Q3 = pointsArray[Math.min(pointsArray.length - 1, indexQ3 - 1)];
                    long IQR = Q3 - Q1;

                    long L = Q1 - (long) (1.5 * IQR);
                    long U = Q3 + (long) (1.5 * IQR);

                    int count = 0;
                    if (boundType.equals("U")) {
                        for (long point : pointsArray) {
                            if (point > U) {
                                count++;
                            }
                        }
        ...
        if (obj instanceof ParticipantRange) {
            ...
            if (participantId >= range.startId && participantId <= range.endId) {
                ...
                if (participantId > range.startId) {
                    ParticipantRange leftRange = new ParticipantRange(range.startId, participantId - 1, range.points, team);
                    participants.put(leftRange.startId, leftRange);
                    team.participants.add(leftRange);
                }

                if (participantId < range.endId) {
                    ParticipantRange rightRange = new ParticipantRange(participantId + 1, range.endId, range.points, team);
    ...
    private static void updateParticipantPoints(Participant participant, int delta) {
        ...
        team.pointCounts.put(oldPoints, team.pointCounts.get(oldPoints) - 1);
        if (team.pointCounts.get(oldPoints) == 0) {
    ...
    private static void removeTopParticipants(Team team, TreeMap<Integer, Object> participantsMap, int count) {
        // Collect all participants (individual and ranges) and flatten them
        MyLinkedList<Participant> participantList = new MyLinkedList<>();
        for (int i = 0; i < team.participants.size(); i++) {
            ...
            if (obj instanceof Participant) {
                ...
                ParticipantRange pr = (ParticipantRange) obj;
                // Split the range into individual participants
