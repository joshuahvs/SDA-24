...
public class Cobacobalagi {
            ...
            if (command == 'A') {
                ...
                if (customerQueue.isEmpty()) {
                    ...
                    if (customerUntukDilayani.budget == hargaIkanYangDapatDibeli) {
                        long hargaUntukDibayar = (!kuponStack.isEmpty()) ?
                                Math.max(1, hargaIkanYangDapatDibeli - kuponStack.pop()) : hargaIkanYangDapatDibeli;
                ...
                out.println(Math.abs(budget - hargaTerdekat));
            } else if (command == 'L') {
                long id = Long.parseLong(parts[1]);
                Customer customer = idToCustomer.remove(id); // Remove from map first
                out.println((customer == null) ? -1 : customer.budget);
                if (customer != null) {
                ...
                if (tipeQuery == 1) {
                    ...
                    int bestState = (dp[0][budget] == maxHappiness) ? 0 :
                            (dp[1][budget] == maxHappiness ? 1 : 2);
    ...
    static long findClosestFishPrice(long[] fishPrices, long budget) {
        ...
        // Handle edge cases and compare closest prices
        return (right < 0) ? fishPrices[0] :
                (left >= fishPrices.length) ? fishPrices[fishPrices.length - 1] :
                (budget - fishPrices[right] <= fishPrices[left] - budget) ? fishPrices[right] : fishPrices[left];
    }
