...
public class TP1pCopyCopycapek {
    ...
    public static void main(String[] args) throws IOException {
        ...
        PriorityQueue<Customer> customerQueue = new PriorityQueue<>(new Comparator<Customer>() {
            public int compare(Customer c1, Customer c2) {
                ...
                return (c1.budget != c2.budget) ? Long.compare(c2.budget, c1.budget) :
                        (c1.patience != c2.patience) ? Long.compare(c1.patience, c2.patience) :
                        Long.compare(c1.id, c2.id);
            }
        });
        ...
        for (int q = 0; q < Q; q++) {
            ...
            if (command == 'A') {
                ...
                if (customerQueue.isEmpty()) {
                    ...
                    // Handle case when patience is 0
                    ...
                    // Common steps for resetting patience and re-adding the customer if applicable
                    if (hargaIkanYangDapatDibeli >= 0) {
                ...
                Customer customer = idToCustomer.remove(id); // Remove from map first
                if (customer == null) {
                    out.println(-1);
                } else {
                    out.println(customer.budget);
                    customerQueue.remove(customer); // Only attempt queue removal if customer exists
                }
            } else if (command == 'D') {
                ...
                if (tipeQuery == 1){
                    if (currentMaxBudget<budget){
                        precomputeSouvenirDP(hargaSuvenir, kebahagiaanSuvenir, budget);
                        currentMaxBudget = budget;
                    }
    ...
    static long findClosestFishPrice(long[] fishPrices, long budget) {
        ...
        // Binary search for closest fish price
        while (left <= right) {
        ...
        // Handle edge cases
        if (right < 0) {
            return fishPrices[0];  // Budget is smaller than the smallest fish price
        }
        if (left >= fishPrices.length) {
            return fishPrices[fishPrices.length - 1];  // Budget is larger than the largest fish price
        }
    
        // Compare closest prices on the left and right side
        return (budget - fishPrices[right] <= fishPrices[left] - budget) ? fishPrices[right] : fishPrices[left];
    }
