...
public class TP3test12 {
    ...
    // Metode untuk menghitung MST dengan prioritas jalan yang terhubung langsung ke ID
    static int computeMSTWithID(int ID) {
        List<EdgeFull> allEdges = new ArrayList<>();

        // Mengumpulkan semua jalan unik
        for (int u = 1; u <= V; u++) {
            for (Edge edge : adjList[u]) {
                if (u < edge.to) { // Menghindari duplikasi
                    allEdges.add(new EdgeFull(u, edge.to, edge.weight));
                }
            }
        }

        // Memisahkan jalan yang terhubung langsung ke ID dan yang lainnya
        List<EdgeFull> connectedToID = new ArrayList<>();
        List<EdgeFull> others = new ArrayList<>();
        ...
        // Mengurutkan jalan yang terhubung langsung ke ID berdasarkan berat
        connectedToID.sort(Comparator.comparingInt(e -> e.weight));

        // Mengurutkan jalan lainnya berdasarkan berat
        others.sort(Comparator.comparingInt(e -> e.weight));

        // Menggabungkan kedua list, jalan yang terhubung ke ID diutamakan
        List<EdgeFull> sortedEdges = new ArrayList<>();
        sortedEdges.addAll(connectedToID);
