...
public class CobaJdoang{
    ...
    public static void main(String[] args) {
        ...
        for (int q = 0; q < Q; q++) {
            ...
            if (command == 'R') {
                ...
                int totalDistance = queryJ(id); 
                out.println(totalDistance); // Output: 14
            }
        }
    ...
    static int queryJ(int id) {
        List<EdgeFull> allEdges = new ArrayList<>();
    
        // Tambahkan semua edge ke dalam list allEdges, KECUALI edge yang terhubung langsung dengan id
        for (int i = 1; i <= V; i++) {
            for (Edge edge : adjList[i]) {
                if (i < edge.to && i != id && edge.to != id) { 
                    allEdges.add(new EdgeFull(i, edge.to, edge.weight));
                }
            }
        }
    
        // Tambahkan "forced edges" dari kota id ke semua tetangganya
        for (Edge edge : adjList[id]) {
            allEdges.add(new EdgeFull(id, edge.to, edge.weight));
        }
    
        // Urutkan edge berdasarkan weight
