...
public class CobaRdoang{
    ...
    static int computeMSTWithID(int ID) {
        ...
        for (int u = 1; u <= V; u++) {
            for (Edge edge : adjList[u]) {
                // Agar setiap edge tidak duplikat, hanya ambil jika u < edge.to
                if (u < edge.to) {
        ...
        // Urutkan semua edge berdasarkan bobot, dengan prioritas pada edge yang terhubung dengan ID
        allEdges.sort((e1, e2) -> {
            boolean e1ConnectedToID = e1.from == ID || e1.to == ID;
            boolean e2ConnectedToID = e2.from == ID || e2.to == ID;
    
            if (e1ConnectedToID && !e2ConnectedToID) {
                return -1; // e1 diprioritaskan
            } else if (!e1ConnectedToID && e2ConnectedToID) {
                return 1; // e2 diprioritaskan
            } else {
                return Integer.compare(e1.weight, e2.weight); // Urutkan berdasarkan bobot
            }
        });
    
        // Union-Find untuk Kruskal (tetap sama)
