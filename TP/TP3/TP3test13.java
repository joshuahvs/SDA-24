...
public class TP3test13 {
    ...
    static int computeMSTWithID(int ID) {
        ...
        // Pisahkan edge yang terhubung langsung ke ID dan yang lainnya
        ...
        // Edge yang terhubung ke ID dimasukkan semua
        // Edge lain diurutkan berdasarkan bobot agar kita dapat memilih edge minimal
        ...
        // Masukkan semua edge yang terhubung ke ID
        int total = 0;
        int edgesUsed = 0;
        for (EdgeFull e : connectedToID) {
        ...
        // Tambahkan edge-edge lain yang diperlukan untuk memastikan semua terhubung
        for (EdgeFull e : others) {
            int pu = find(parent, e.from);
            int pv = find(parent, e.to);
            if (pu != pv) {
                parent[pu] = pv;
