...
public class TP3test11 {
    ...
    static Map<String, Integer> reachableCache = new HashMap<>();
    ...
    public static void main(String[] args) {
        ...
        for (int q = 0; q < Q; q++) {
            ...
            if (command == 'R') {
                ...
                int reachableCities = bfsReachable(currentCity, energy);
    ...
    static int bfsReachable(int startCity, int initialEnergy) {
        boolean[] visited = new boolean[V + 1];
        Queue<State> queue = new LinkedList<>(); 
        // State adalah kelas untuk menyimpan kota dan energi saat ini
    
        queue.offer(new State(startCity, initialEnergy));
        ...
        while (!queue.isEmpty()) {
            ...
            for (Edge edge : adjList[current.city]) {
                int nextCity = edge.to;
                int nextEnergy = current.energy - edge.weight;
    
                if (!visited[nextCity] && nextEnergy >= 0) {
                    visited[nextCity] = true;
                    reachableCities++;
                    queue.offer(new State(nextCity, initialEnergy)); // Isi ulang energi
                }
            }
        }
