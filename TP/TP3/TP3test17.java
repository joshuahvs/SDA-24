...
public class TP3test17 {
    ...
    public static void main(String[] args) {
        ...
        for (int q = 0; q < Q; q++) {
            ...
            if (command == 'R') {
                ...
                int id = Integer.parseInt(parts[1]);
                out.println(mstWithForcedEdges(id)); 
            }
        }
    ...
    // Method untuk menghitung MST dengan forced edges
    static int mstWithForcedEdges(int id) {
        List<EdgeFull> edges = new ArrayList<>();

        // Tambahkan semua edge ke list edges
        for (int u = 1; u <= V; u++) {
            for (Edge edge : adjList[u]) {
                int v = edge.to;
                int weight = edge.weight;
                // Pastikan setiap edge hanya ditambahkan sekali
                if (u < v) { 
                    edges.add(new EdgeFull(u, v, weight));
                }
            }
        }

        // Urutkan edges berdasarkan weight
        edges.sort(Comparator.comparingInt(e -> e.weight));

        UnionFind uf = new UnionFind(V);
        int totalWeight = 0;

        // Hubungkan semua edge yang terhubung langsung dengan kota id
        for (EdgeFull edge : edges) {
        ...
        // Lanjutkan algoritma Kruskal seperti biasa
        for (EdgeFull edge : edges) {
            if (uf.union(edge.from, edge.to)) {
                totalWeight += edge.weight;
            }
    ...
    // Kelas UnionFind untuk algoritma Kruskal
